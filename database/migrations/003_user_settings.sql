...
CREATE INDEX idx_session_logs_user ON session_logs(user_id, login_at DESC);
CREATE INDEX idx_session_logs_active ON session_logs(user_id) WHERE is_active = true;
CREATE INDEX idx_session_logs_suspicious ON session_logs(is_suspicious) WHERE is_suspicious = true;
...
CREATE INDEX idx_2fa_enabled ON two_factor_auth(user_id) WHERE enabled = true;
...
CREATE INDEX idx_audit_log_severity ON security_audit_log(severity, created_at DESC) WHERE severity IN ('warning', 'critical');
CREATE INDEX idx_audit_log_metadata ON security_audit_log USING gin(metadata);
...
CREATE INDEX idx_password_reset_valid ON password_reset_tokens(token_hash) WHERE is_valid = true AND used_at IS NULL;
...
CREATE INDEX idx_email_verification_valid ON email_verification_tokens(token_hash) WHERE is_valid = true AND verified_at IS NULL;
