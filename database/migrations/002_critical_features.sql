...
CREATE INDEX idx_funding_txns_user ON funding_transactions(user_id, created_at DESC);
CREATE INDEX idx_funding_txns_status ON funding_transactions(status) WHERE status IN ('pending', 'processing');
...
CREATE INDEX idx_kyc_docs_user ON kyc_documents(user_id, uploaded_at DESC);
...
CREATE INDEX idx_screener_cache_hash ON screener_cache(criteria_hash);
CREATE INDEX idx_screener_cache_expires ON screener_cache(expires_at);
...
CREATE INDEX idx_price_alerts_symbol ON price_alerts(symbol, is_active);
CREATE INDEX idx_price_alerts_active ON price_alerts(is_active, triggered_at, expires_at);
...
CREATE INDEX idx_alert_triggers_user ON alert_triggers(user_id, trigger_time DESC);
...
CREATE INDEX idx_news_importance ON news_articles(importance DESC, published_at DESC);
...
CREATE UNIQUE INDEX idx_market_movers_unique ON market_movers(snapshot_date, mover_type, symbol);
CREATE INDEX idx_market_movers_date ON market_movers(snapshot_date DESC);
...
CREATE INDEX idx_performance_metrics_account ON portfolio_performance_metrics(account_id, period_days);
...
CREATE INDEX idx_generated_statements_user ON generated_statements(user_id, generated_at DESC);
CREATE INDEX idx_generated_statements_account ON generated_statements(account_id, statement_type);
CREATE INDEX idx_generated_statements_period ON generated_statements(period_start, period_end);
...
CREATE INDEX idx_geo_news_published ON geo_news_events(published_at DESC);
CREATE INDEX idx_geo_news_country ON geo_news_events(country_code, published_at DESC);
CREATE INDEX idx_geo_news_impact ON geo_news_events(impact_level DESC, published_at DESC);
CREATE INDEX idx_geo_news_location ON geo_news_events(latitude, longitude);
CREATE INDEX idx_geo_news_symbols ON geo_news_events USING GIN(symbols_affected);
...
CREATE INDEX idx_faq_category ON faq_items(category) WHERE is_published = true;
CREATE INDEX idx_faq_search ON faq_items USING gin(search_vector);
...
CREATE INDEX idx_tickets_status ON support_tickets(status, priority) WHERE status NOT IN ('resolved', 'closed');
...
CREATE INDEX idx_statements_user ON account_statements(user_id, period_end DESC);
...
CREATE INDEX idx_alerts_active ON price_alerts(symbol, status) WHERE status = 'active';
