...
def test_vectorized_backtest_sharpe_annualization_is_sqrt_252():
    ...
    # Note: run_vectorized_backtest shifts signal by 1, so the first return is missed (position=0).
    # The returns vector becomes [0.0, -0.005, 0.002, 0.0, 0.004]
    r = np.array([0.0, -0.005, 0.002, 0.0, 0.004], dtype=np.float64)
