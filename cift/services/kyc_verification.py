...
class KYCVerificationService:
    ...
    async def verify_document(
        ...
        try:
            ...
            logger.success(f"Document verification completed: {verification_status} ({confidence_score:.2f})")
            return result
        ...
        try:
            ...
            for doc in docs:
                ...
                document_checks[doc_id] = {
                    ...
                    "details": json.loads(doc['verification_details']) if doc['verification_details'] else {}
                }
            ...
            logger.success(f"Identity verification completed: {verification_status} ({overall_score:.2f})")
            return result
    ...
    def _calculate_identity_score(
        ...
        verified_docs = sum(
            1 for doc_id, doc in document_checks.items()
    ...
    def _determine_identity_status(
        ...
        has_verified_doc = any(
            doc['status'] == 'verified'
            for doc in document_checks.values()
        )
...
async def verify_document_async(document_id: UUID, document_type: str) -> DocumentVerificationResult:
    ...
    async with pool.acquire() as conn:
        ...
        for user_row in pending_users:
            try:
                ...
                logger.info(f"Processed identity for user {user_row['user_id']}: {result.verification_status}")
            except Exception as e:
                logger.error(f"Failed to process identity for user {user_row['user_id']}: {e}")

    logger.success(f"Processed {len(pending_docs)} documents and {len(pending_users)} identity verifications")
