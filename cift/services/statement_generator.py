...
class StatementGeneratorService:
    ...
    async def generate_monthly_statement(
        self, user_id: UUID, account_id: UUID, year: int, month: int
    ) -> GeneratedStatement:
        ...
        period = StatementPeriod(
            start_date=period_start, end_date=period_end, period_type="monthly"
        )
    ...
    async def generate_trade_confirmation(
        ...
        trade_data = {
            ...
            "trade_details": {
                ...
                "total_value": float(order["filled_quantity"] or 0)
                * float(order["average_price"] or 0),
            },
        }
    ...
    async def generate_tax_document(
        self, user_id: UUID, account_id: UUID, tax_year: int, document_type: str = "1099"
    ) -> GeneratedStatement:
        """Generate tax documents (1099, consolidated tax summary)."""

        period = StatementPeriod(
            start_date=date(tax_year, 1, 1), end_date=date(tax_year, 12, 31), period_type="annual"
        )
    ...
    async def _gather_statement_data(
        self, user_id: UUID, account_id: UUID, period: StatementPeriod
    ) -> StatementData:
    ...
    async def _gather_tax_data(
        self, user_id: UUID, account_id: UUID, tax_year: int
    ) -> dict[str, Any]:
    ...
    async def _generate_pdf_statement(self, data: StatementData, title: str) -> bytes:
        ...
        account_data = [
            [
                "Account Holder:",
                f"{data.user_info.get('first_name', '')} {data.user_info.get('last_name', '')}",
            ],
