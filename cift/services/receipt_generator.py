...
class ReceiptGenerator:
    ...
    async def generate_receipt(
        ...
        try:
            logger.info(
                f"Generating receipt for transaction: {transaction_data.get('id', 'unknown')}"
            )
        ...
        line_drawing.add(
            Line(0, 1, 6.5 * inch, 1, strokeColor=ReceiptGenerator.PRIMARY_BLUE, strokeWidth=1.5)
        )
        ...
        doc_id_style = ParagraphStyle(
            "DocID", parent=subtitle_style, fontSize=7, alignment=TA_RIGHT
        )
        ...
        status_color = (
            ReceiptGenerator.SUCCESS_GREEN
            if status.lower() == "completed"
            else ReceiptGenerator.TEXT_GRAY
        )
        ...
        left_col_data.append(
            [
                Paragraph(
                    f'<font color="{status_color.hexval()}"><b>{transaction_type_display.upper()}</b></font>',
                    value_style,
                )
            ]
        )
        ...
        amount_color = (
            ...
            else ReceiptGenerator.DANGER_RED
        )
        ...
        right_col_data.append(
            [
                Paragraph(
                    f'<para align="center"><font size="8" color="#64748b">{transaction_type_display}</font></para>',
                    value_style,
                )
            ]
        )
        ...
        breakdown_data = [
            [
                Paragraph("Subtotal", amount_label_style),
                Paragraph(f"${amount:,.2f}", amount_value_style),
            ],
            [
                Paragraph("Processing Fee", amount_label_style),
                Paragraph(f"${fee:,.2f}", amount_value_style),
            ],
            [
                Paragraph(
                    "<b>Total</b>",
                    ParagraphStyle(
                        "BoldLabel", parent=amount_label_style, fontName="Helvetica-Bold"
                    ),
                ),
                Paragraph(
                    f'<font color="{ReceiptGenerator.PRIMARY_BLUE.hexval()}"><b>${total_amount:,.2f}</b></font>',
                    amount_value_style,
                ),
            ],
        ]
        ...
        main_table = Table(
            main_layout, colWidths=[3.1 * inch, 3.4 * inch], spaceBefore=0, spaceAfter=0
        )
        main_table.setStyle(
            TableStyle(
                [
        ...
        footer_line.add(
            Line(0, 0, 6.5 * inch, 0, strokeColor=ReceiptGenerator.BORDER_GRAY, strokeWidth=1)
        )
