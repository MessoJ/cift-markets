...
class GatedResidualNetwork(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        context_dim: Optional[int] = None,  # For conditioning on external signal
    ):
        super().__init__()
...
class VariableSelectionNetwork(nn.Module):
    ...
    def forward(
        ...
        processed = [grn(x) for grn, x in zip(self.feature_grns, inputs)]
...
class OrderFlowTransformer(nn.Module):
    ...
    def forward(
        ...
        tick_mask: Optional[torch.Tensor] = None,
        second_mask: Optional[torch.Tensor] = None,
        minute_mask: Optional[torch.Tensor] = None,
        ...
        for i, (x, embed) in enumerate(zip(timeframe_inputs, self.feature_embeds)):
            # Embed
            x_emb = embed(x)
        ...
        encoded = []
        for i, (x_emb, encoder, mask) in enumerate(zip(embedded, self.encoders, masks)):
