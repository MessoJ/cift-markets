...
class SumOfExponentialsKernel(nn.Module):
    ...
    def __init__(
        ...
        init_betas: List[float] = [10.0, 1.0, 0.1],  # Fast, medium, slow
    ):
        super().__init__()
    ...
    def forward(
        ...
        dt_expanded = dt.unsqueeze(-1).unsqueeze(0)  # [1, batch, num_events, 1]
...
class PowerLawApproxKernel(nn.Module):
    ...
    def compute_intensity_increment(
        ...
        dt: torch.Tensor,
        event_types: torch.Tensor,
        prev_intensity: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
...
class HawkesOrderFlowModel(nn.Module):
    ...
    def forward(
        ...
        event_sizes: Optional[torch.Tensor] = None,  # [batch, num_events]
        current_time: Optional[torch.Tensor] = None, # [batch]
    ...
    def compute_log_likelihood(
        ...
        event_times: torch.Tensor,
        event_types: torch.Tensor,
        event_sizes: Optional[torch.Tensor] = None,
