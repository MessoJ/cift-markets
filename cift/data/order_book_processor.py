...
if NUMBA_AVAILABLE:
    ...
    def compute_weighted_imbalance_numba(bid_prices, bid_sizes, ask_prices, ask_sizes, mid_price, levels):
        ...
        ask_weighted = sum(
            ...
            for price, size in zip(ask_prices[:levels], ask_sizes[:levels], strict=False)
    ...
    def compute_vpin_numba(buy_volumes, sell_volumes, bucket_volume):
        ...
        if n == 0:
            return 0.0
        total_imbalance = sum(abs(b - s) for b, s in zip(buy_volumes, sell_volumes, strict=False))
        denominator = n * bucket_volume
...
class OrderBookProcessor:
    ...
    def _compute_snapshot(self, timestamp: int) -> OrderBookSnapshot:
        ...
        if self._trade_signs:
            ...
            trade_flow = sum(s * sz for s, sz in zip(recent_signs, recent_sizes, strict=False))
        else:
            trade_flow = 0.0
