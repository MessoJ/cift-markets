...
async def get_news_arcs(
    ...
    connection_type: Optional[str] = Query(None, regex="^(trade|impact|correlation|all)$"),
    db: asyncpg.Connection = Depends(get_db),
):
    """
...
async def search_globe_data(
    ...
    if categories:
        param_count += 1
        where_clauses.append(f"a.category = ANY(${param_count})")
...
async def get_country_details(
    ...
    try:
        ...
        time_threshold = datetime.utcnow() - time_windows.get(timeframe, timedelta(hours=24))
