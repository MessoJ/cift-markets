...
async def submit_application(
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            await conn.execute(
                "UPDATE users SET alpaca_account_id = $1 WHERE id = $2", alpaca_id, user_id
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Brokerage account creation failed: {str(e)}"
            ) from e
...
async def get_verification_status(
    ...
    async with pool.acquire() as conn:
        ...
        return {
            ...
            "requirements": {
                "identity_document": (
                    kyc_profile["identity_document_uploaded"] if kyc_profile else False
                ),
...
async def accept_agreements(
    ...
    if not all(
        [request.terms_accepted, request.privacy_accepted, request.risk_disclosure_accepted]
    ):
