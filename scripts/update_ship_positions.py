...
async def update_all_ships():
    ...
    try:
        ...
        if updated_count == 0 and simulated_count > 0:
            logger.warning(f"⚠️  No live API data - using simulation")
            logger.warning(f"   Configure API keys for real-time tracking:")
            logger.warning(f"   - AISSTREAM_API_KEY (https://aisstream.io)")
            logger.warning(f"   - MARINETRAFFIC_API_KEY (https://marinetraffic.com)")
